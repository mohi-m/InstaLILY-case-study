...
CREATE INDEX IF NOT EXISTS part_chunks_embedding_idx
    ON part_chunks USING ivfflat (embedding vector_cosine_ops) WITH (lists = 10);
...
CREATE INDEX IF NOT EXISTS symptoms_embedding_idx
    ON symptoms USING ivfflat (name_embedding vector_cosine_ops) WITH (lists = 10);
...
CREATE INDEX IF NOT EXISTS model_qa_embedding_idx
    ON model_qa USING ivfflat (question_embedding vector_cosine_ops) WITH (lists = 10);
